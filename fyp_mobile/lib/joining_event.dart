...
class JoiningEventPage extends StatefulWidget {
  

  JoiningEventPage({Key? key}) : super(key: key);

  @override
  State<JoiningEventPage> createState() => JoiningEventPageState();
}


class JoiningEventPageState extends  State<JoiningEventPage> {
  List results = [];
                                    ...
                                    Column(
                                      ...
                                      children: [
                                        ...
                                        OutlinedButton(
                                          onPressed: (){
                                            print(results[index]);
                                            print(results[index] is QueryDocumentSnapshot);
                                            showWithdrawalConfirmDialog(context, results[index]);
                                          },
                                          style: ElevatedButton.styleFrom(
                                            minimumSize: const Size(100, 40),
                                            padding: const EdgeInsets.symmetric(horizontal: 16),
 ...
 void showWithdrawalConfirmDialog(BuildContext context, QueryDocumentSnapshot event) {
    // set up the button
  ...
  void withdrawFromEvent(QueryDocumentSnapshot event) {
    var data = event.data();
    FirebaseFirestore.instance
    .collection("Events")
    .doc(event.id)
    ...
    .where("user", 
      ...
      .then((value) {
        value.docs.forEach((element) {
          ...
          .delete()
          .then((value) {
            showToast(context, "You have withdraw from the event");
          },
          onError: (e) {
            showToast(context, "Fail to withdraw from the event");
          }
          );
        });
      });
